...
const FONT_SIZES = {
  active:   "clamp(22px, 3vw, 20px)",
  inactive: "clamp(18px, 1.8vw, 16px)",
};
...
export const Navbar = forwardRef<{ setActiveIndex: (index: number) => void }, NavbarProps>(
  ({ scrollContainerRef, isClickScrolling }, ref) => {
    ...
    return (
      <div className="flex flex-col text-end items-end gap-4 select-none">
        {MENU.map((item, index) => (
          <div
            ref={(el) => {
            ...
            className="interactive transition-colors duration-200 origin-right"
            key={item.name}
